...
Model::~Model()
{
	...
	if (normalmap_tangent) delete normalmap; normalmap = NULL;
...
void Model::create_map(const char* filename)
{
	...
	texfile = texfile.substr(0, dot) + std::string("_normal_tangent.tga");
	if (_access(texfile.data(), 0) != -1)
	{
		normalmap_tangent = new TGAImage();
		load_texture(filename, "_normal_tangent.tga", normalmap_tangent);
	}
...
vec3 Model::normal_mapping(vec2 uv)
{
	uv[0] = fmod(uv[0], 1);
	uv[1] = fmod(uv[1], 1);
	...
	vec3 res;
	for (int i = 0; i < 3; i++)
		res[2 - i] = static_cast<float>(c[i]) / 255.f * 2.f - 1.f; //because the normap_map coordinate is -1 ~ +1
	return res;
}
